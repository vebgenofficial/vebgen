...
from typing import List, Dict, Any, Optional, Literal, Union, ForwardRef, Set, TypedDict # Keep Literal, Add Union, ForwardRef
...
class FeatureTask(BaseModel):
    ...
    def validate_deps_list(cls, v):
        ...
        if isinstance(v, str):
            ...
            deps = [str(dep).strip() for dep in v if dep is not None] # Ensure dep is not None
        else:
...
class ProjectFeature(BaseModel):
    ...
    work_log: List[str] = Field(default_factory=list, description="A log of actions and results from the agent's execution for this feature.") # BUG FIX #6
...
class PythonFunction(BaseModel):
    ...
    is_async: bool = False
    channel_layer_invocations: List[str] = Field(default_factory=list, description="Detects calls like channel_layer.group_send.")
...
class DjangoModel(PythonClass): # Inherits name, bases, methods, attributes
    ...
    is_mptt_model: bool = Field(default=False, description="True if the model inherits from MPTTModel for tree structures.")

class WagtailPage(DjangoModel):
    """Represents a Wagtail Page model, inheriting from DjangoModel."""
    # Wagtail-specific fields can be added here, e.g., content_panels
    content_panels: List[str] = Field(default_factory=list)

class DjangoCMSPlugin(DjangoModel):
    """Represents a django-cms Plugin model, inheriting from DjangoModel."""
...
class DjangoModelFileDetails(PythonFileDetails):
    ...
    wagtail_pages: List[WagtailPage] = Field(default_factory=list)
    cms_plugins: List[DjangoCMSPlugin] = Field(default_factory=list)
...
class DjangoView(PythonFunction): # Inherits name, params, decorators, return_type_hint
    ...
    # --- New: More detailed view attributes ---
    ...
    queryset_optimizations: List[str] = Field(default_factory=list, description="List of QuerySet optimization methods used, like 'select_related' or 'prefetch_related'.")
    uses_raw_sql: bool = Field(default=False, description="True if the view uses .raw() or .extra() for SQL queries.")
    aggregations_annotations: List[str] = Field(default_factory=list, description="List of fields used in .annotate() or .aggregate().")

class DjangoViewFileDetails(PythonFileDetails):
    """Structured representation of a Django views.py file."""
    drf_viewsets: List['DRFViewSetDetails'] = Field(default_factory=list)
...
class DjangoURLPattern(BaseModel):
    ...
    path_converters: Dict[str, str] = Field(default_factory=dict) # e.g., {"pk": "int"}
...
class DjangoURLInclude(BaseModel):
    ...
    namespace: Optional[str] = None # The namespace argument of include()

class DRFRouterRegistration(BaseModel):
    """Represents a `router.register()` call for a DRF ViewSet."""
    prefix: str
    viewset_name: str
    basename: Optional[str] = None

class DRFViewSetDetails(BaseModel):
    """Structured representation of a DRF ViewSet class."""
    name: str
    queryset_model: Optional[str] = None
    serializer_class: Optional[str] = None
    authentication_classes: List[str] = Field(default_factory=list)
    permission_classes: List[str] = Field(default_factory=list)
    pagination_class: Optional[str] = None
...
class DjangoURLConfDetails(PythonFileDetails):
    ...
    drf_routers: List[DRFRouterRegistration] = Field(default_factory=list)

class DjangoSerializerField(BaseModel):
    """Represents a field within a DRF Serializer."""
    name: str
    field_type: str
    source: Optional[str] = None
    read_only: bool = False

class DjangoSerializer(PythonClass):
    """Represents a Django REST Framework Serializer."""
    meta_model: Optional[str] = None
    meta_fields: List[str] = Field(default_factory=list)
    serializer_fields: List[DjangoSerializerField] = Field(default_factory=list)
...
class DjangoAdminClass(PythonClass): # Inherits name, bases, methods, attributes
    ...
    fieldsets: List[Any] = Field(default_factory=list, description="Stores the complex nested structure of fieldsets.")
    inlines: List[str] = Field(default_factory=list, description="List of inline class names used.")

class DjangoAdminInline(PythonClass):
    """Represents an inline admin class (e.g., TabularInline, StackedInline)."""
    model: Optional[str] = None
    extra: int = 1 # Default value for 'extra' attribute

class DjangoTestClass(PythonClass):
    """Represents a test class, extending PythonClass with test-specific details."""
    has_setup_test_data: bool = False
    uses_request_factory: bool = False
    uses_api_client: bool = False
...
class DjangoAdminFileDetails(PythonFileDetails):
    ...
    inline_classes: List[DjangoAdminInline] = Field(default_factory=list)
...
class DjangoTestFileDetails(PythonFileDetails): # New model for tests.py
    ...
    test_classes: List[DjangoTestClass] = Field(default_factory=list)

class CeleryBeatSchedule(BaseModel):
    """Represents a single entry in the CELERY_BEAT_SCHEDULE."""
    task_name: str
    schedule: str # e.g., "crontab(minute='*/15')" or "timedelta(seconds=30)"
...
class DjangoSettingsDetails(PythonFileDetails):
    ...
    env_vars_used: Dict[str, str] = Field(default_factory=dict) # e.g., {"SECRET_KEY": "SECRET_KEY"}
    celery_beat_schedules: List[CeleryBeatSchedule] = Field(default_factory=list)
    ...
    asset_pipeline_tools_detected: List[str] = Field(default_factory=list)

class DjangoTemplateTag(BaseModel):
    """Represents a custom template tag or filter."""
    name: str
    tag_type: Literal["simple_tag", "inclusion_tag", "filter"]
    takes_context: bool = False
    template_path: Optional[str] = None # For inclusion_tag

class DjangoTemplateTagFileDetails(PythonFileDetails):
    """Structured representation of a templatetags file."""
    tags_and_filters: List[DjangoTemplateTag] = Field(default_factory=list)

class DjangoSignalReceiver(PythonFunction):
    """Represents a function decorated as a signal receiver."""
    signal: str
    sender: Optional[str] = None
    dispatch_uid: Optional[str] = None

class DjangoSignalFileDetails(PythonFileDetails):
    """Structured representation of a signals.py file."""
    receivers: List[DjangoSignalReceiver] = Field(default_factory=list)

class CeleryTask(PythonFunction):
    """Represents a Celery task function."""
    task_options: Dict[str, Any] = Field(default_factory=dict) # e.g., {"bind": True, "max_retries": 3}
    uses_retry: bool = Field(default=False, description="True if the task body calls self.retry().")

class CeleryTaskFileDetails(PythonFileDetails):
    """Structured representation of a Celery tasks.py file."""
    tasks: List[CeleryTask] = Field(default_factory=list)

class DjangoChannelsConsumer(PythonClass):
    """Represents a Django Channels consumer class."""
    pass # For now, just identifies the class. Methods are already in PythonClass.

class DjangoChannelsRouting(PythonFileDetails):
    """Structured representation of a Django Channels routing.py file."""
    websocket_patterns: List[DjangoURLPattern] = Field(default_factory=list)

class DjangoMigrationOperation(BaseModel):
    """Represents a single operation within a Django migration file."""
    type: str # e.g., "CreateModel", "AddField", "RunPython"
    # More details could be added here, e.g., model_name, field_name

class DjangoMigrationDetails(BaseModel):
    """Structured representation of a Django migration file."""
...
class GraphQLField(BaseModel):
    ...
    name: str
    field_type: str  # e.g., "graphene.String", "graphene.List(UserType)"
    args: Dict[str, str] = Field(default_factory=dict)

class GraphQLType(PythonClass):
    """Represents a GraphQL ObjectType, Query, or Mutation."""
    graphql_fields: List[GraphQLField] = Field(default_factory=list)

class GraphQLSchemaDetails(PythonFileDetails):
    """Structured representation of a GraphQL schema.py file."""
    queries: List[GraphQLType] = Field(default_factory=list)
    mutations: List[GraphQLType] = Field(default_factory=list)
    object_types: List[GraphQLType] = Field(default_factory=list)
...
class FileStructureInfo(BaseModel):
    ...
    file_type: Literal[
        ...
        "django_migration", "django_serializer", "django_signal", "celery_task",
        "django_form", "django_admin", "django_settings", "django_apps_config", "django_graphql_schema",
    ...
    django_view_details: Optional[DjangoViewFileDetails] = Field(default=None, description="Details for a views.py file, including DRF ViewSets.")
    # Add a field for DRF ViewSet details, if you decide to parse them separately
    # drf_viewset_details: Optional[DRFViewSetDetails] = None 
    ...
    django_serializer_details: Optional['DjangoSerializerFileDetails'] = None # New
    django_templatetag_details: Optional[DjangoTemplateTagFileDetails] = None # New
    django_signal_details: Optional[DjangoSignalFileDetails] = None # New
    celery_task_details: Optional[CeleryTaskFileDetails] = None # New
...
class DjangoSerializerFileDetails(PythonFileDetails):
    """Structured representation of a Django serializers.py file."""
    serializers: List[DjangoSerializer] = Field(default_factory=list)

class DjangoChannelsFileDetails(PythonFileDetails):
    """Structured representation of a Django Channels consumers.py file."""
    consumers: List[DjangoChannelsConsumer] = Field(default_factory=list)
...
class ProjectStructureMap(BaseModel):
    ...
    global_files: Dict[str, "FileStructureInfo"] = Field(default_factory=dict) # For root files like manage.py, utils.py
    global_url_registry: Dict[str, GlobalURLRegistryEntry] = Field(default_factory=dict) # url_name: GlobalURLRegistryEntry
    middleware_classes: List[str] = Field(default_factory=list) # List of middleware class paths
...
class CommandOutput(TypedDict):
    """A simple dictionary for the raw output of a command execution."""
    command: str
    exit_code: int
    stdout: str
    stderr: str
